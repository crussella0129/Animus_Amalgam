...
- **Result (final head, see test report):** `tests/evals/test_local_qualification_policy.py`
  14 passed; `tests/agent/test_minimum_context_explicit_local.py` 6 passed.
...
| `qualification_receipt_audit` (privacy half) → `…::test_published_evidence_excludes_private_paths_and_credentials` | P1 | pass | The published receipts and all manifests contain no drive-letter or home user paths, no `Bearer` values and no 48-hex ephemeral tokens. |
